...
from services.unified_progress_service import unified_progress_service, ProgressInfo
from datetime import datetime

router = APIRouter()


class ProgressUpdateServiceAdapter:
    """适配器，将 unified_progress_service 适配到旧的 progress_update_service 接口"""
    
    def get_task_progress(self, task_id: str) -> Dict[str, Any]:
        """
        获取任务进度
        注意：unified_progress_service 基于 project_id，这里返回一个空字典
        实际进度信息应该从 Task 模型中获取
        """
        return {}
    
    def get_all_active_tasks(self) -> Dict[str, Dict[str, Any]]:
        """获取所有活动任务"""
        # unified_progress_service 不提供此功能
        return {}


progress_update_service = ProgressUpdateServiceAdapter()
