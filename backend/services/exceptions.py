...
class ProcessingError(ServiceError):
    ...
    def __init__(self, message: str, step_name: Optional[str] = None, project_id: Optional[str] = None, details: Optional[Dict[str, Any]] = None, cause: Optional[Exception] = None):
        details = details or {}
        if step_name:
...
class LLMError(ServiceError):
    ...
    def __init__(self, message: str, provider: Optional[str] = None, details: Optional[Dict[str, Any]] = None, cause: Optional[Exception] = None):
        if provider:
            details = details or {}
...
class LLMTimeoutError(LLMError):
    ...
    def __init__(self, message: str = "LLM调用超时", provider: Optional[str] = None, timeout_seconds: Optional[int] = None, details: Optional[Dict[str, Any]] = None, cause: Optional[Exception] = None):
...
class LLMRateLimitError(LLMError):
    ...
    def __init__(self, message: str = "LLM调用频率限制", provider: Optional[str] = None, retry_after: Optional[int] = None, details: Optional[Dict[str, Any]] = None, cause: Optional[Exception] = None):
...
class VideoProcessingError(ProcessingError):
    ...
    def __init__(self, message: str, video_path: Optional[str] = None, step_name: Optional[str] = None, details: Optional[Dict[str, Any]] = None, cause: Optional[Exception] = None):
        if video_path:
            details = details or {}
            details["video_path"] = video_path
        super().__init__(message, step_name, details, cause)
...
def handle_service_error(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            try:
                # 使用项目根目录下的 logs 目录
                debug_log_path = Path(__file__).parent.parent.parent / "logs" / "debug.log"
                debug_log_path.parent.mkdir(parents=True, exist_ok=True)
                with open(debug_log_path, 'a', encoding='utf-8') as f:
                    f.write(error_msg + '\n\n')
            except Exception as log_error:
                logger.warning(f"无法写入调试日志: {log_error}")
