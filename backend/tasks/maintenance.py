...
@shared_task(bind=True, name='backend.tasks.maintenance.cleanup_logs')
def cleanup_logs_task(self) -> Dict[str, Any]:
    ...
    try:
        from services.log_cleanup_service import cleanup_logs
...
def backup_project_data(self, project_id: str, backup_path: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            from core.path_utils import get_project_directory, get_backup_directory
            if not backup_path:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                backup_dir = get_backup_directory() / f"{project_id}_{timestamp}"
            else:
