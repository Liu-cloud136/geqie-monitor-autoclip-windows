  ...
  const deleteTask = useCallback(async (taskId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('❌ 删除任务失败:', response.status, response.statusText);
        return false;
      }
    } catch (error) {
  ...
  const loadProjectTasks = useCallback(async (projectId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const projectTasks = data.data?.tasks || data || [];
        console.log('📋 获取到任务数量:', projectTasks.length);

        setTasks(prev => {
          const newTasks = new Map(prev);
          projectTasks.forEach((task: any) => {
            console.log('📝 添加任务:', task.task_id || task.id, task.status, task.progress);
            newTasks.set(task.task_id || task.id, {
              id: task.task_id || task.id,
              status: task.status as TaskStatus['status'],
              progress: task.progress || 0,
              message: task.name || task.message,
